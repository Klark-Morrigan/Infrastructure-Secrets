...
1. Call `Invoke-ModuleInstall` (from `Common.PowerShell`) for
...
`Common.PowerShell` must be declared in `RequiredModules` in
...
- `Infrastructure.Secrets.psd1` — add `Common.PowerShell` to
