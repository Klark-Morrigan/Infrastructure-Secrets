...
Reason: unlike `Common.PowerShell`, this repo has no PS 5.1 code compromises to
...
| File | Change |
|------|--------|
| `Infrastructure.Secrets/Infrastructure.Secrets.psd1` | `PowerShellVersion` 5.1 -> 7.0; add `CompatiblePSEditions = @('Core')`; bump `RequiredModules` Common.PowerShell to 2.0.0; bump `ModuleVersion` (breaking change - major bump recommended: 3.0.0) |
